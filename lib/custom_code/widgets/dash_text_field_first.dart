...
class DashTextFieldFirstState extends State<DashTextFieldFirst> {
  ...
  void clearTextFieldValue() {
    setState(() {
      _controller.clear();
      FFAppState().update(() {
        // FFAppState().lostFocusValue = "" as int;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: TextFormField(
        ...
        onChanged: (value) {
          setState(() {
            // _textFieldValueF = int.parse(value);
...
class _controllerValidator {
  final empcodeMask = MaskTextInputFormatter(mask: '#########');
}
